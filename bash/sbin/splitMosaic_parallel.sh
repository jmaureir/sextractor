#!/bin/bash
#
# split mosaic image in individual CCDs using in parallel way
# individual instances of imcopy (single threaded version of imcopy)
# detached in parallel in the same compute node via xargs
#
...
if [ $NUM_CCDS -gt 0 ]; then
    # create a list of ccds to be processed with xargs
    ccd_list=`mktemp`
    ...
    do
        ...
        echo "$ccd_num $CCD_FILENAME" >> $ccd_list
    done
    ...
    extract_ccd() {
        ...
        if [ $EXIT_CODE -ne 0 ]; then
            echo "imcopy exit with error code $EXIT_CODE"
            exit $EXIT_CODE
        fi
    }
    export -f extract_ccd
    cat $ccd_list | xargs -n 1 -P $NUM_CCDS -d '\n' -I {} bash -c 'extract_ccd $@' _ {}
    rm $ccd_list
